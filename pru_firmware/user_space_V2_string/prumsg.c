...
int main(int argc, char *argv[]) {		// argv es un array de punteros char. Y argc es un entero con el número de punteros (partes del comando).
	...
	int fd, wr, rd, idx, val;		// fd: Valor de retorno de la función open(). Entero que representa
	...
	// Zero message buffer.
	memset(msg_buff, '\0', RPMSG_BUF_SIZE);			// Inicializa a cero msg_buff.
	...
	/*
	// Si se proporcionan datos, se convierten a entero y se escriben en el buffer.
	if (argc == 4) {
		val = atoi(argv[3]);						// Convierto el string del puntero 3 de argv a entero. ( es donde está el valor de cambio)
	...
	*/	
		...
		int cont;
		int i;
		tamaño = strlen(argv[3]);
		i = 0;
		
		for(cont = 1; cont <= strlen(argv[3]); cont++) {
			msg_buff[cont] = argv[3][i];
			i++;
	...
	// Escribe el dato a la PRU.
		wr = write(fd, msg_buff, tamaño + 1);					// Escribe n bytes, donde n son los 5 bytes que hemos introducido en el array msg_buff.
	} else {
	...
	rd = read(fd, msg_buff, RPMSG_BUF_SIZE);
	while(rd == 0) {
			rd = read(fd, msg_buff, RPMSG_BUF_SIZE);
	}
	...
	/* Este fragmento es para cuando en la PRU 1 se declara el mensaje en unsigned int, yo lo he cambiado a int.
